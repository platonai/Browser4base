...
class DataStorageFactory(
    ...
    val dataStoreClassName: String? = null
) {
    private val pageStoreClass: Class<out DataStore<String, GWebPage>> get() = detectDataStoreClass(dataStoreClassName, conf)

    private var _dataStore: DataStore<String, GWebPage>? = null

    var healthyChecker: () -> Boolean = { true }
     ...
     */
    fun detectDataStoreClassName(dataStoreClassName: String? = null, conf: ImmutableConfig): String {
        if (!AppContext.isActive) {
            throw IllegalApplicationStateException("Inactive application context")
        }

        if (!dataStoreClassName.isNullOrBlank()) {
        ...
        if (dataStoreClass != FILE_BACKEND_STORE_CLASS && !healthyChecker()) {
            logger.warn("Data store class {} is not healthy, fallback to {}", dataStoreClass, FILE_BACKEND_STORE_CLASS)
            return FILE_BACKEND_STORE_CLASS
        }
    ...
    fun <K, V : Persistent> detectDataStoreClass(dataStoreClassName: String?, conf: ImmutableConfig): Class<out DataStore<K, V>> {
        return Class.forName(detectDataStoreClassName(dataStoreClassName, conf)) as Class<out DataStore<K, V>>
    }
